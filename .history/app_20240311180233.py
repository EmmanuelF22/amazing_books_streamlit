...
with col2:
    ...
    with st.form(key='params_for_api'):
        ...
        comment = st.text_input('Write a comment...', 'excellent book')
        st.set_option('deprecation.showfileUploaderEncoding', False)
        uploaded_file = st.file_uploader("... or upload Choose a CSV file", type="csv")
        if uploaded_file is not None:
            comment = pd.read_csv(uploaded_file)
            st.write(comment)
